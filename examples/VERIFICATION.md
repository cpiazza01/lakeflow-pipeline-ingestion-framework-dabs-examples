...
Replace `<catalog>` with your target catalog (e.g. `enterprise_dev`).

---

## Patients — SCD Type 1 (Upsert)

**Silver table:** `<catalog>.silver.patients`  
...
```sql
-- Row count
SELECT COUNT(*) FROM <catalog>.silver.patients;
-- Expected: 60

-- No duplicate member_id
SELECT member_id, COUNT(*) AS n
FROM <catalog>.silver.patients
GROUP BY member_id
HAVING n > 1;
-- Expected: 0 rows
```
...
SELECT COUNT(*) FROM <catalog>.silver.patients WHERE marital_status = 'M';
```
...
SELECT DISTINCT LEFT(ssn, 7) FROM <catalog>.silver.patients LIMIT 5;
-- Expected: '***-**-' for users outside the pii-authorized group

-- Address masking: non-pii-authorized users should see '*** MASKED ***'
SELECT DISTINCT address FROM <catalog>.silver.patients LIMIT 5;
...
**Silver table:** `<catalog>.silver.encounters`  
...
```sql
-- Total row count
SELECT COUNT(*) FROM <catalog>.silver.encounters;
-- Expected: 4,930
...
**Silver table:** `<catalog>.silver.providers`  
...
SELECT COUNT(*) FROM <catalog>.silver.providers;
-- Expected: 52
...
FROM <catalog>.silver.providers p1
JOIN <catalog>.silver.providers p2
...
WHERE provider_id IN (
  SELECT provider_id FROM <catalog>.silver.providers
...
**Silver table:** `<catalog>.silver.observations`  
...
FROM <catalog>.silver.observations o
JOIN (
  SELECT member_id, loinc_code, MAX(observation_ts) AS max_ts
  FROM <catalog>.bronze.observations
  WHERE type = 'numeric'
  GROUP BY member_id, loinc_code
...
SELECT observation_ts FROM <catalog>.silver.observations
